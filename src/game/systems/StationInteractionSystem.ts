...
export const EMPTY_STATION_INTERACTION: StationInteractionSnapshot = {
  ...
  stationId: null,
  stationName: "",
  stationState: "No spacecraft nearby",
  ownershipState: "none",
  stationLevel: 0,
  health: 0,
  maxHealth: 0,
...
export function createStationInteractionSnapshot({
  ...
  if (unclaimed) {
    ...
    actions.push({ id: "open_command", label: "Spacecraft Systems", kind: "open_command", enabled: landed, hotkey: landed ? "U" : undefined, priority: 100, lockReason: landed ? "" : "Dock inside the spacecraft to access upgrade systems." });
    actions.push({
      id: "deposit",
      label: "Deposit Cargo",
      kind: "deposit",
    ...
    actions.push({ id: "move_base", label: "WASD Pilot", kind: "move_base", enabled: false, priority: 55, lockReason: landed ? station.localRelocationAvailable ? "Use WASD while docked to fly the spacecraft." : station.localRelocationReason : "Dock before moving the spacecraft." });
    actions.push({ id: "loadout", label: "Manage Loadout", kind: "loadout", enabled: landed, priority: 45, lockReason: landed ? "" : "Dock to access spacecraft loadout systems." });
  } else {
    actions.push({ id: "scan", label: "Scan Spacecraft", kind: "scan", enabled: false, priority: 20, lockReason: "This spacecraft belongs to another crew." });
  }
