...
export const STATION_CONFIG = {
  ...
  baseRadius: 430,
  starterStationDistance: 1400,
  localRelocationStepDistance: 4200,
  stationBasePilotSpeed: 135,
  stationPilotSpeedPerBoosterLevel: 34,
  stationPilotAcceleration: 260,
  stationPilotDrag: 1.85,
...
export function createLandingPads(): LandingPad[] {
  return Array.from({ length: 6 }, (_, index) => {
    ...
    return {
      ...
      unlocked: index === 0,
      level: 1,
      positionOffset: { x: Math.cos(angle) * 520, y: Math.sin(angle) * 520 },
    };
  });
}
